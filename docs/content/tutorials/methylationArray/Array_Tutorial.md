---
title: "Methylation Array Data Analysis"
...
In this workflow, we will provide examples of the steps involved in analyzing methylation array data using R and Bioconductor. The different steps include: reading in the raw data, quality control, filtering, different normalization methods and probe-wise differential methylation analysis. Additional approaches such as differential methylation analysis of regions, differential variability analysis, gene ontology analysis and estimating cell type composition will also be presented. Finally, we will provide some examples of useful ways to visualise methylation array data.

# Load Packages

Start the analysis by loading the set of R packages that will be needed during the downstream analysis. _limma_ provides the statistical framework for testing differential methylation. _minfi_, _missMethyl_, _minfiData_ and _DMRcate_ are packages developed to work with methylation data. _Gviz_ and _RColorBrewer_ provide functions for the visualization of the data.
